...
describe("Delta Pack Reflection", () => {
  ...
  describe("Schema Structure - Containers", () => {
    it("should generate correct schema for arrays", () => {
      class WithArrays {
        @ArrayType(globalThis.String)
        strings: string[] = [];

        @ArrayType(globalThis.Number)
        numbers: number[] = [];
      }
    ...
    it("should generate correct schema for arrays with number modifiers", () => {
      class WithModifiedArrays {
        @ArrayType(globalThis.Number, { unsigned: true })
        unsignedInts: number[] = [];

        @ArrayType(globalThis.Number, { float: true })
        floats: number[] = [];

        @ArrayType(globalThis.Number, { float: 0.1 })
    ...
    it("should generate correct schema for maps with number modifiers", () => {
      class WithModifiedMaps {
        @RecordType(StringType(), globalThis.Number, { unsigned: true })
        unsignedMap: Map<string, number> = new Map();

        @RecordType(StringType(), globalThis.Number, { float: 0.01 })
    ...
    it("should generate correct schema for optionals", () => {
      class WithOptionals {
        @OptionalType(globalThis.String)
        optString?: string;

        @OptionalType(globalThis.Number)
        optNumber?: number;

        @OptionalType(globalThis.Number, { unsigned: true })
        optUnsigned?: number;

        @OptionalType(globalThis.Number, { float: 0.1 })
        optFloat?: number;
      }
  ...
  describe("Schema Structure - Nested Containers", () => {
    it("should generate correct schema for array of arrays (int[][])", () => {
      class Matrix {
        @ArrayType(ArrayType(globalThis.Number))
        data: number[][] = [];
      }
    ...
    it("should generate correct schema for nested containers with modifiers", () => {
      class NestedWithModifiers {
        @ArrayType(ArrayType(globalThis.Number, { float: 0.01 }))
        floatMatrix: number[][] = [];

        @RecordType(StringType(), ArrayType(globalThis.Number, { unsigned: true }))
        vectorMap: Map<string, number[]> = new Map();

        @OptionalType(ArrayType(globalThis.Number))
    ...
    it("should generate correct schema for deeply nested containers (int[][][])", () => {
      class Cube {
        @ArrayType(ArrayType(ArrayType(globalThis.Number)))
        data: number[][][] = [];
      }
