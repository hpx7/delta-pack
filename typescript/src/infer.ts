...
export type Infer<
  T extends Type,
  S extends Record<string, Type> = {},
  D extends number = 10
> = [D] extends [never] ? unknown :
  T extends { type: "string" } ? string :
  T extends { type: "int" } ? number :
  T extends { type: "uint" } ? number :
  T extends { type: "float" } ? number :
  T extends { type: "boolean" } ? boolean :
  T extends { type: "enum"; options: readonly (infer U)[] } ? U :
  T extends { type: "array"; value: infer V } ?
    (V extends Type ? Array<Infer<V, S, Prev[D]>> : never) :
  T extends { type: "optional"; value: infer V } ?
    (V extends Type ? Infer<V, S, Prev[D]> | undefined : never) :
  T extends { type: "record"; key: infer K; value: infer V } ?
    (K extends Type ? (V extends Type ? Map<Infer<K, S, Prev[D]>, Infer<V, S, Prev[D]>> : never) : never) :
  T extends { type: "reference"; reference: infer R } ?
    (R extends keyof S ? Infer<S[R], S, Prev[D]> : unknown) :
...
type InferUnion<
  Options extends readonly { reference: string }[],
  S extends Record<string, Type>,
  D extends number
> = {
